...
st.set_page_config(page_title="TDA with Macro‑Parameterised Filtrations", layout="wide")
...
st.sidebar.markdown(f"**Base max distance:** {config.BASE_MAX_DISTANCE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'tda_macro_' in f]
    if not json_files:
        return None
...
st.sidebar.markdown("---")
st.sidebar.caption("TDA with Macro‑Parameterised Filtrations | Macro‑scaled Rips complex")
